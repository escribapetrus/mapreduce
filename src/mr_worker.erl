...
put_function(Pid, F) -> gen_server:call(Pid, {put_function, F}).

put_key(Pid, Key) -> gen_server:call(Pid, {put_key, Key}).
...
handle_call({put_function, F}, _, Data) ->
    {reply, F, Data#mr_worker{function=F}};

handle_call({put_key, Key}, _, Data) ->
    {reply, Key, Data#mr_worker{input = [Key | Data#mr_worker.input]}}.
